	...
	foreach( $a as $ai )
	{
		...
		foreach( _Cp_XPathCssQuery( $ctx, $xpath, $selCss ) as $item )
		{
			...
			foreach( Gen::GetArrField( $ai, array( 'bps' ), array() ) as $dim => $aiR )
			{
				...
				{
					if( Gen::GetArrField( $aiR, array( 'vertical' ), false ) )
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c { ' . $alignItems . 'display: grid; grid-template-rows:repeat(' . $nShow . ', 1fr); }' . "\n";
					else
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c { ' . $alignItems . 'display: grid; grid-template-columns:repeat(' . $nShow . ', 1fr); }' . "\n";
