...
function CssExtractImports( &$cont )
{
...
{
	$contHead = implode( '', array_merge( CssExtractImports( $cont ), $imports ) );
	if( $contHead )
		$cont = $contHead . $cont;
...
function Style_ProcessCont( &$ctxProcess, $sett, $settCache, $settContPr, $settCss, $settImg, $settCdn, $head, &$item, $srcInfo, $src, $id,  $cont, $contAdjusted, $isInlined, $status, $isNoScript, &$contGroups, $optLoad = null )
{
	...
	if( $group )
	{
		$contImports = CssExtractImports( $cont );
...
function Style_ProcessCont_ItemApply( &$ctxProcess, $sett, $settCache, $settCss, $settCdn, $head, &$item, $srcInfo, $src, $id,  $cont, $contAdjusted, $isInlined, $status, $isNoScript, $repos, $composite = false, $optLoad = null )
{
	...
	if( $inline )
	{
		...
		if( ( $cont = $ctxProcess[ 'cbs' ] -> ContPostProc( 'css', $cont, false ) ) === false )
			return( false );
	...
	else
	{
		...
		{
			if( $composite && !$ctxProcess[ 'cbs' ] -> GetContentProcessorForce( $sett ) && ($ctxProcess[ 'chunksEnabled' ]??null) && Gen::GetArrField( $settCache, array( 'chunks', 'css' ) ) )
			{
				if( ( $cont = $ctxProcess[ 'cbs' ] -> ContPostProc( 'css', $cont, true ) ) === false )
					return( false );
		...
		if( !($ctxProcess[ 'compatView' ]??null) && $optLoad && !$isNoScript && $status != 'crit' )
		{
			...
			if( $status == 'fonts' && !Gen::GetArrField( $settCss, array( 'font', 'delayNonCritWithJs' ) ) )
			{
...
function _EmbedStyles_Process( $processor, &$ctxProcess, $sett, $settCache, $settContPr, $settCss, $settImg, $settCdn, $doc, &$aCritFonts, &$aImgSzAlternativesBlocksGlobal = null )
{
	...
	foreach( $processor -> xpath -> query( './/*[@style]', $ctxProcess[ 'ndBody' ] ) as $item )
	{
		if( $ctxProcess[ 'cbs' ] -> IsAborted( $ctxProcess, $settCache ) ) return;

		if( in_array( $item -> nodeName, array( 'script', 'noscript', 'style', 'img', 'picture', 'source' ) ) )
		    continue;
		...
		if( isset( $ctxProcess[ '_stat' ][ 'styleEmbCount' ] ) )
			$ctxProcess[ '_stat' ][ 'styleEmbCount' ] += 1;

			$res = _EmbedStyles_Process_AdjustCont( $itemClassIdx, $item, $style, $processor, $ctxProcess, $settCache, $settCss, $settImg, $settCdn, $doc, $aCritFonts, $aImgSzAlternativesBlocksGlobal );
			if( $res === false )
				return( false );
...
function _EmbedStyles_Process_AdjustCont( &$itemClassIdx, $item, &$style, $processor, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $doc, &$aCritFonts, &$aImgSzAlternativesBlocksGlobal = null )
{
	if( !( $ctxProcess[ 'mode' ] & 4 ) )
	{
		...
		$r = StyleProcessor::AdjustContFast( $ctxProcess, $settCache, $settCss, $settImg, $settCdn, '', $style, true );
		if( $r === false )
			return( false );
	}
	else
...
function Styles_Process( &$ctxProcess, $sett, $settCache, $settContPr, $settCss, $settImg, $settCdn, $doc )
{
	...
	{
		$ctxProcess[ '_stat' ][ 'styleEmbCount' ] = 0;
		$ctxProcess[ '_stat' ][ 'styleInlCount' ] = 0;
		$ctxProcess[ '_stat' ][ 'styleSrcCount' ] = 0;
	}
	...
	for( $i = 0; $i < count( $items ); $i++ )
	{
		...
		if( !$isInlined )
		{
			if( isset( $ctxProcess[ '_stat' ][ 'styleSrcCount' ] ) )
				$ctxProcess[ '_stat' ][ 'styleSrcCount' ] += 1;

			$rel = ( string )HtmlNd::GetAttrVal( $item, 'rel' );
			if( $cont === null )
				if( $rel != 'stylesheet' )
				{
		...
		else
		{
			if( isset( $ctxProcess[ '_stat' ][ 'styleInlCount' ] ) )
				$ctxProcess[ '_stat' ][ 'styleInlCount' ] += 1;
		}
		...
		if( $cont === null )
		{
			if( !$isInlined )
			{
				...
				{
					if( ($ctxProcess[ 'remote' ]??null) && $ctxProcess[ 'cbs' ] -> PreFetchLocalFiles( array( $srcInfo[ 'filePath' ] => array( 'filePathRoot' => ($srcInfo[ 'filePathRoot' ]??null), 'cssExtract' => true ) ) ) === false )
						return( false );
			...
			{
				$contWoImports = $cont;
				$imports = CssExtractImports( $contWoImports );
				if( $imports )
				{
					$media = $item -> getAttribute( 'media' );
	...
	if( ( $ctxProcess[ 'mode' ] & 4 ) && isset( $ctxProcess[ 'lrnDsc' ] ) && !$processor -> writeLrnData( $ctxProcess, $ctxProcess[ 'lrnDsc' ], $ctxProcess[ 'lrnDataPath' ] ) )
		return( false );
...
class StyleProcessor
{
	...
	function AdjustContBegin( &$ctxProcess, $settCache, &$cont )
	{
		if( !isset( $ctxProcess[ 'lrnDsc' ] ) )
			return( new AnyObj() );
		...
		$ctxAdjust = new AnyObj();
		$ctxAdjust -> aDepsPrev = $ctxProcess[ 'deps' ]; $ctxProcess[ 'deps' ] = array();
		$ctxAdjust -> aXpathSelCachePrev = $this -> _aXpathSelCache; $this -> _aXpathSelCache = array();
		$ctxAdjust -> contHash = $contHash;
		return( $ctxAdjust );
	}
	...
	function AdjustCont( $extract, &$aCritFonts, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, &$cont, $isInlined )
	{
		...
		$ctxAdjust = $this -> AdjustContBegin( $ctxProcess, $settCache, $cont );
		if( !( $ctxAdjust instanceof AnyObj ) )
			return( $ctxAdjust );
		...
		if( $this -> AdjustContEnd( $ctxAdjust, $res, $ctxProcess, $settCache, $cont ) === false )
			return( false );

		return( $res );
	}

	function AdjustContEnd( $ctxAdjust, $res, &$ctxProcess, $settCache, &$cont )
	{
		if( !isset( $ctxProcess[ 'lrnDsc' ] ) )
			return( null );

		$aDeps = $ctxProcess[ 'deps' ]; $ctxProcess[ 'deps' ] = $ctxAdjust -> aDepsPrev; unset( $ctxAdjust -> aDepsPrev ); DepsAddMany( $ctxProcess[ 'deps' ], $aDeps );
		$aXpathSelCache = $this -> _aXpathSelCache; $this -> _aXpathSelCache = $ctxAdjust -> aXpathSelCachePrev; unset( $ctxAdjust -> aXpathSelCachePrev ); self::_XpathSelCacheAdd( $this -> _aXpathSelCache, $aXpathSelCache );
		...
		if( ( $ctxProcess[ 'mode' ] & 4 ) )
		{
			...
			$this -> _aAdjustContCache[ $ctxAdjust -> contHash ] = $aInfo;
		}
	...
	function _AdjustCont( $extract, &$aCritFonts, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, &$cont, $isInlined )
	{
		...
		if( !( $ctxProcess[ 'mode' ] & 4 ) )
		{
			...
			$r = StyleProcessor::AdjustContFast( $ctxProcess, $settCache, $settCss, $settImg, $settCdn, $src, $cont, $isInlined );
			if( $r === false )
				return( false );
		...
		foreach( ( $aCritFonts !== null ? array( 'main', 'fonts' ) : array( '' ) ) as $stage )
		{
			foreach( $cssDoc -> getContents() as $i )
			{
				...
				if( $ctxProcess[ 'cbs' ] -> IsAborted( $ctxProcess, $settCache ) ) return( null );
			}
		}
	...
	static function AdjustContFast( &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $src, &$cont, $isInlined )
	{
		$adjusted = null;
	...
	static function CssExtactUrlsFast( $cont )
	{
		$a = array();
			...
			if( !$stage || $stage == 'fonts' )
			{
				...
				else
				{
					if( ($settCss[ 'delayNonCritWithJs' ]??null) )
					{
						...
						{
							$ctxFont = self::_CreateFoncFaceCtx( $block );
							$bNonCritExcl = false;

							foreach( Gen::GetArrField( $settCss, array( 'font', 'nonCritExcl' ), array() ) as $fontNonCritExclExpr )
							{
								if( ExprConditionsSet_Match( $fontNonCritExclExpr, array( $ctxFont, 'cbMatch' ) ) )
								{
									$bNonCritExcl = true;
									break;
								}
							...
							else
							{
								...
								{
									foreach( $block -> getRules( 'src' ) as $oValSrc )
									{
										...
										{
											$src = array(); self::_GetCssRuleValUrlObjs( $oSrc, $src );
											if( $src )
											{
												$src = trim( $src[ 0 ] -> getURL() -> getString() );
												$src = str_replace( '::bhkdyqcetujyi::', (

													( false && !$isInlined  ) ) ? $ctxProcess[ 'siteDomainUrl' ] : '', $src );

												if( !Ui::IsSrcAttrData( $src ) )
												{
													$ctxProcess[ 'lazyloadStylesNonCritFont' ] = array( 'family' => $ctxFont -> getProp( 'family' ), 'src' => $src );
													break;
												}
											}
										}
	...
	private static function _CreateFoncFaceCtx( $ruleSet )
	{
		$ctxFont = new AnyObj();
	...
	static function AdjustRuleSet( $ruleSet, &$aDepFonts, $adjustes, $doc, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, $isInlined )
	{
		...
		if( $ruleSet instanceof Sabberworm\CSS\RuleSet\AtRuleSet && $ruleSet -> atRuleName() == 'font-face' )
		{
			...
			$ctxFont = self::_CreateFoncFaceCtx( $ruleSet );
					...
					if( $aFmt )
					{
						...
						if( !Ui::IsSrcAttrData( $url ) )
						{
							...
							if( isset( $srcInfo[ 'filePath' ] ) )
								$contFont = $ctxProcess[ 'cbs' ] -> ReadLocalFile( $srcInfo[ 'filePath' ], ($srcInfo[ 'filePathRoot' ]??null) );
	...
	static function GetFirstImportSimpleAttrs( $ctxProcess, $import, $src )
	{
		...
		foreach( $cssDoc -> getContents() as $block )
		{
			if( $block instanceof Sabberworm\CSS\Property\Import )
			{
				...
				$res = array( 'url' => $url, 'filePath' => ($srcInfo[ 'filePath' ]??null), 'filePathRoot' => ($srcInfo[ 'filePathRoot' ]??null) );
				if( count( $args ) > 1 )
	...
	static function getLrnNeededData( &$aData, &$aLrnGlob, $dsc, $dataPath )
	{
		...
		{
			$aLrnGlob[] = 'css/xslb/' . $id . '.dat.gz';
		}
	}

	static function mergeLrnData( $ctxProcess, &$dsc, $dscPrev, $dataPath )
	{
		if( ($dsc[ 'css' ][ 'c' ]??null) === ($dscPrev[ 'css' ][ 'c' ]??null) )
			return( null );

		$aCssSelIsCritCachePrev = array(); $aAdjustContCachePrev = array(); self::_readLrnData( $aCssSelIsCritCachePrev, $aAdjustContCachePrev, $ctxProcess, $dscPrev, $dataPath );
		$aCssSelIsCritCache = array(); $aAdjustContCache = array(); self::_readLrnData( $aCssSelIsCritCache, $aAdjustContCache, $ctxProcess, $dsc, $dataPath );
		return( self::_writeLrnData( array_merge( $aCssSelIsCritCache, $aCssSelIsCritCachePrev ), array_merge( $aAdjustContCache, $aAdjustContCachePrev ), $ctxProcess, $dsc, $dataPath ) );
	}

	function readLrnData( $ctxProcess, $dsc, $dataPath, $bLearning )
	{
		self::_readLrnData( $this -> _aCssSelIsCritCache, $this -> _aAdjustContCache, $ctxProcess, $dsc, $dataPath, $bLearning );
	}

	function writeLrnData( $ctxProcess, &$dsc, $dataPath )
	{
		return( self::_writeLrnData( $this -> _aCssSelIsCritCache, $this -> _aAdjustContCache, $ctxProcess, $dsc, $dataPath ) );
	}

	private static function _readLrnData( &$aCssSelIsCritCache, &$aAdjustContCache, $ctxProcess, $dsc, $dataPath, $bLearning = false )
	{
		if( ( !$bLearning || isset( $dsc[ 's' ] ) ) && ( $id = Gen::GetArrField( $dsc, array( 'css', 'c' ) ) ) )
		{
			$data = Tof_GetFileDataCb( array( $ctxProcess[ 'cbs' ], 'Tof_GetFileDataEx' ), $dataPath . '/css/c', 'dat.gz', array( 3, function( $data, $vFrom ) { return( $data ); } ), true, $id );
			$v = Gen::GetArrField( $data, array( 'v' ), 1 );

			if( isset( $dsc[ 's' ] ) )
			{
				$aCssSelIsCritCache = Gen::GetArrField( $data, array( 'sc' ), array() );
			...
			if( !$bLearning )
			{
				...
				else
				{
					...
					if( $id = Gen::GetArrField( $dsc, array( 'css', 'xslb' ) ) )
						$aXpathSelCacheOld = Gen::GetArrField( Tof_GetFileDataCb( array( $ctxProcess[ 'cbs' ], 'Tof_GetFileDataEx' ), $dataPath . '/css/xslb', 'dat.gz', 1, true, $id ), array( 'd' ), array() );

					$aAdjustContCacheOld = Gen::GetArrField( $data, array( 'ac' ), array() );
					$aAdjustContCache = array();
					foreach( $aAdjustContCacheOld as $contHash => $res )
						$aAdjustContCache[ $contHash ] = array( 'c' => $res, 'd' => Gen::GetArrField( $data, array( 'd' ), array() ), 'xslb' => $aXpathSelCacheOld );
				}
			}
		}
	}

	private static function _writeLrnData( $aCssSelIsCritCache, $aAdjustContCache, $ctxProcess, &$dsc, $dataPath )
	{
		if( ( isset( $dsc[ 's' ] ) && $aCssSelIsCritCache ) || $aAdjustContCache )
		{
			$data = array();

			if( isset( $dsc[ 's' ] ) && $aCssSelIsCritCache )
				$data[ 'sc' ] = array_filter( $aCssSelIsCritCache, function( $v ) { return( !!$v ); } );

			if( $aAdjustContCache )
				$data[ 'ac' ] = $aAdjustContCache;

			$dsc[ 'css' ][ 'c' ] = '';
			if( Gen::HrFail( Tof_SetFileDataCb( array( $ctxProcess[ 'cbs' ], 'Tof_SetFileDataEx' ), $dataPath . '/css/c', 'dat.gz', $data, 3, TOF_COMPR_MAX, $dsc[ 'css' ][ 'c' ] ) ) )
				return( false );
		}
		else
			unset( $dsc[ 'css' ][ 'c' ] );
