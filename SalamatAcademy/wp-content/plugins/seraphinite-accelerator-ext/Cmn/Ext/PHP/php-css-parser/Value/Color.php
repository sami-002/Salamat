	...
	public static function parse(ParserState $oParserState) {
		...
		if ($oParserState->comes('#')) {
			...
			for ($i = 0; ; ++$i) {
				...
				if( $oParserState->comes('from', true) )
				{
					$oParserState->consume(4, false);
					$oParserState->consumeWhiteSpace();
					$aColor[''] = new ValuePrefixed(Value::parsePrimitiveValue($oParserState), 'from');
					$i--;
				}
				else
				{
		...
		{
			if( $sSeparator == '/' )
				$aColor[$sColorMode[strlen($sColorMode) - 1]] = new ValuePrefixed($aColor[$sColorMode[strlen($sColorMode) - 1]], '/');
			$sSeparator = ' ';
		}
		else if ($nSeps > 1)
			$sSeparator = ',';
